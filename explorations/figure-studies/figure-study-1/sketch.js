...
const SPHERE_RADIUS_MM = 22;    // max bounding radius
const NUM_CURVES = 24;          // rings per shape
const POINTS_PER_CURVE = 120;   // resolution per ring
const VERTICAL_SQUASH = 1.0;    // 1.0 = perfect silhouette aspect
const RING_TILT = 0.3;          // perspective tilt
...
function generateSingleShape(cx, cy) {
    const shapeCurves = [];
    ...
    // If it's a square, we rotate it by ~45 degrees usually looks best
    const baseRotation = (type === 'CUBE' || type === 'PYRAMID') ? (Math.PI / 4) : 0;
    ...
    for (let i = 0; i < NUM_CURVES; i++) {
        ...
        if (type === 'SPHERE') {
            ...
            yOffset = v * SPHERE_RADIUS_MM;
            bandRadius = Math.sqrt(Math.max(0, SPHERE_RADIUS_MM * SPHERE_RADIUS_MM - yOffset * yOffset));
            ...
            yOffset = v * SPHERE_RADIUS_MM * heightFactor;
            
            // Constant radius
            bandRadius = SPHERE_RADIUS_MM * 0.85; 
            ...
            bandRadius = SPHERE_RADIUS_MM * 0.9 * raw; 
...
function generateCurvesGeometry() {
    ...
    const usableW = A4_WIDTH - 2 * GRID_MARGIN_MM;
    // Determine cell size based on width
    const cellSide = usableW / GRID_COLS;
    
    // Calculate vertical margin to center the square-ish grid block
    const totalGridHeight = cellSide * GRID_ROWS;
    const topMargin = (A4_HEIGHT - totalGridHeight) / 2;

    for (let row = 0; row < GRID_ROWS; row++) {
        for (let col = 0; col < GRID_COLS; col++) {
            const cx = GRID_MARGIN_MM + cellSide * (col + 0.5);
            const cy = topMargin + cellSide * (row + 0.5);
            
            const shape = generateSingleShape(cx, cy);
            curvesMm.push(shape);
        }
    }
}
...
function exportToSVG() {
    ...
    link.download = `a4-canvas-variant-shapes-${date}.svg`;
    link.href = url;
