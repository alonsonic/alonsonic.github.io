...
// const SPHERE_RADIUS_MM = 22;    // max bounding radius -> Removed in favor of dynamic calculation
...
function generateSingleShape(cx, cy, size) {
    ...
    const buildup = 0.05 + Math.random() * 0.95;
    ...
    for (let i = 0; i < NUM_CURVES; i++) {
        ...
        const noiseScale = buildup * (0.05 + 0.15 * densityFactor);
...
function generateCurvesGeometry() {
    ...
    // Calculate grid layout with square cells to ensure uniform padding
    // We want uniform spacing between items.
    // Let's define a fixed padding between items, e.g. 5mm
    const ITEM_PADDING_MM = 10;
    
    // Calculate usable width for actual shapes
    // Total Width = (Cols * ItemWidth) + ((Cols - 1) * Padding) + (2 * Margin)
    // We know Width, Margin, Padding, Cols. Solve for ItemWidth.
    // 210 = (3 * w) + (2 * 5) + (2 * 25)
    // 210 = 3w + 10 + 50
    // 150 = 3w -> w = 50
    
    const totalHPadding = (GRID_COLS - 1) * ITEM_PADDING_MM;
    const totalHMargin = 2 * GRID_MARGIN_MM;
    const availableW = A4_WIDTH - totalHMargin - totalHPadding;
    const itemSize = availableW / GRID_COLS;
    
    // Now for vertical. We want the same itemSize and same padding.
    const totalVPadding = (GRID_ROWS - 1) * ITEM_PADDING_MM;
    const totalGridHeight = (GRID_ROWS * itemSize) + totalVPadding;
    
    // Center vertically
    const topMargin = (A4_HEIGHT - totalGridHeight) / 2;

    for (let row = 0; row < GRID_ROWS; row++) {
        for (let col = 0; col < GRID_COLS; col++) {
            // Calculate center of each cell
            // Left edge of cell = Margin + col * (Size + Padding)
            // Center = Left + Size/2
            
            const xLeft = GRID_MARGIN_MM + col * (itemSize + ITEM_PADDING_MM);
            const cx = xLeft + itemSize / 2;
            
            const yTop = topMargin + row * (itemSize + ITEM_PADDING_MM);
            const cy = yTop + itemSize / 2;
            
            const shape = generateSingleShape(cx, cy, itemSize); // Pass size to shape generator
            curvesMm.push(shape);
        }
    }
}
...
function exportToSVG() {
    ...
    link.download = `figure-study-2-${date}.svg`;
    link.href = url;
