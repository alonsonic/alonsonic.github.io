...
function generateSingleShape(cx, cy, size, shapeType) {
    const shapeCurves = [];
...
function generateCurvesGeometry() {
    ...
    // Outer Cube
    const cubeSize = 120; // mm
    const cube = generateSingleShape(cx, cy, cubeSize, 'CUBE');
    curvesMm.push(cube);

    // Inner Sphere
    const sphereSize = 80; // mm
    const sphere = generateSingleShape(cx, cy, sphereSize, 'SPHERE');
    curvesMm.push(sphere);
}
...
function exportToSVG() {
    ...
    link.download = `figure-study-4-${date}.svg`;
    link.href = url;
